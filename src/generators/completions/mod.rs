...
impl<'key, 'b> ComplGen<'key, 'b> {
    pub fn new(p: &'b Parser<'key, 'b>) -> Self { ComplGen { p: p } }
...
pub fn gen_completions<T: Into<OsString>, S: Into<String>>(&mut self,
                                                            ...
                                                            for_shell: Shell,
                                                            out_dir: T) {
    self.meta.bin_name = Some(bin_name.into());
    self.gen_completions(for_shell, out_dir.into());
}
...
pub fn gen_completions_to<W: Write, S: Into<String>>(&mut self,
                                                        ...
                                                        buf: &mut W) {
    self.meta.bin_name = Some(bin_name.into());
    self.gen_completions_to(for_shell, buf);
}