...
pub struct Validator<'key, 'other, 'z>(&'z mut Parser<'key, 'other>)
    where 'key: 'other,
          'other: 'z;

impl<'key, 'other, 'z> Validator<'key, 'other, 'z> {
    pub fn new(p: &'z mut Parser<'key, 'other>) -> Self { Validator(p) }

    pub fn validate(&mut self,
                    needs_val_of: Option<&'key str>,
    ...
    fn validate_matched_args(&self, matcher: &mut ArgMatcher<'key>) -> ClapResult<()> {
