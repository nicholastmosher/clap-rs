...
pub struct ArgMatcher<'key>(pub ArgMatches<'key>);

impl<'key> Default for ArgMatcher<'key> {
    fn default() -> Self { ArgMatcher(ArgMatches::default()) }
}

impl<'key> ArgMatcher<'key> {
    pub fn new() -> Self { ArgMatcher::default() }

    pub fn propagate(&mut self, arg: &'key str) {
    ...
    pub fn insert(&mut self, name: &'key str) { self.0.args.insert(name, MatchedArg::new()); }
    ...
    pub fn arg_names(&'key self) -> Vec<&'key str> { self.0.args.keys().map(Deref::deref).collect() }

    pub fn entry(&mut self, arg: &'key str) -> Entry<&'key str, MatchedArg> { self.0.args.entry(arg) }

    pub fn subcommand(&mut self, sc: SubCommand<'key>) { self.0.subcommand = Some(Box::new(sc)); }

    pub fn subcommand_name(&self) -> Option<&str> { self.0.subcommand_name() }

    pub fn iter(&self) -> Iter<&str, MatchedArg> { self.0.args.iter() }

    pub fn inc_occurrence_of(&mut self, arg: &'key str) {
    ...
    pub fn inc_occurrences_of(&mut self, args: &[&'key str]) {
    ...
    pub fn add_val_to(&mut self, arg: &'key str, val: &OsStr) {
    ...
    pub fn needs_more_vals<'other, A>(&self, o: &A) -> bool
        where A: AnyArg<'key, 'other>
    {
...
impl<'key> Into<ArgMatches<'key>> for ArgMatcher<'key> {
    fn into(self) -> ArgMatches<'key> { self.0 }
}
