...
/// # Lifetimes
///
/// * `'key`: refers to the lifetime of any [`Arg`] or [`SubCommand`] which must last long enough
/// to be stored in [`ArgMatches<'key>`] as a reference. 
/// * `'other`: refers to all other reference data, such as help messages, value names, etc.
///
/// # Examples
///
/// ```no_run
    ...
    where 'key: 'help
{
    ...
    meta: AppMeta<'other>,
    // Contains all the raw args, including auto-generated help/ver
    args: Vec<Arg<'key, 'other>,
    // Contains all the raw arg groups
...
impl<'key, 'other> App<'key, 'other> {
    ...
    /// print version or help information.
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    #[cfg(feature = "yaml")]
    pub fn from_yaml(yaml: &'key Yaml) -> App<'key, 'key> { App::from(yaml) }
    ...
    /// [`Arg`]: ./struct.Arg.html
    pub fn args_from_usage(mut self, usage: &'key str) -> Self {
    ...
    ///          --major         'keyuto increase major'
    ///          --minor         'keyuto increase minor'
    ///          --patch         'keyuto increase patch'")
    ...
    ///          --minor         'keyuto increase minor'
    ///          --patch         'keyuto increase patch'
    ///          -c [FILE]       'key config file'
    ///          -i [IFACE]      'keyn interface'")
    ///     .groups(&[
    ...
    /// [`ArgGroup`]: ./struct.ArgGroup.html
    /// [`App`]: ./struct.App.html
    pub fn groups(mut self, groups: &[ArgGroup<'key>]) -> Self {
        for g in groups {
    ...
    pub fn get_matches_safe(self) -> ClapResult<ArgMatches<'key>> {
        // Start the parsing
    ...
    /// [`kind`]: ./struct.Error.html
    /// [`AppSettings::NoBinaryName`]: ./enum.AppSettings.html#variant.NoBinaryName
    pub fn get_matches_from_safe<I, T>(mut self, itr: I) -> ClapResult<ArgMatches<'key>>
        where I: IntoIterator<Item = T>,
              T: Into<OsString> + Clone
    {
...
#[cfg(feature = "yaml")]
impl<'key> From<&'key Yaml> for App<'key, 'key> {
    fn from(mut yaml: &'key Yaml) -> Self {
        use args::SubCommand;
...
impl<'key, 'other> AnyArg<'key, 'other> for App<'key, 'other> {
    fn name(&self) -> &'key str {
        unreachable!("App struct does not support AnyArg::name, this is a bug!")
    }
    fn overrides(&self) -> Option<&[&'other str]> { None }
    fn requires(&self) -> Option<&[(Option<&'other str>, &'key str)]> { None }
    fn blacklist(&self) -> Option<&[&'other str]> { None }
    fn required_unless(&self) -> Option<&[&'other str]> { None }
    fn val_names(&self) -> Option<&VecMap<&'other str>> { None }
    fn is_set(&self, _: ArgSettings) -> bool { false }
    fn val_terminator(&self) -> Option<&'other str> { None }
    ...
    fn possible_vals(&self) -> Option<&[&'other str]> { None }
