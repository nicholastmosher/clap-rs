...
#[derive(Default)]
pub struct ArgGroup<'key> {
    #[doc(hidden)]
    pub name: &'key str,
    #[doc(hidden)]
    pub args: Vec<&'key str>,
    #[doc(hidden)]
    pub required: bool,
    #[doc(hidden)]
    pub requires: Option<Vec<&'key str>>,
    #[doc(hidden)]
    pub conflicts: Option<Vec<&'key str>>,
    #[doc(hidden)]
    pub multiple: bool,
}

impl<'key> ArgGroup<'key> {
    ...
    /// # ;
    /// ```
    pub fn with_name(n: &'key str) -> Self {
        ArgGroup {
            name: n,
            required: false,
            args: vec![],
    ...
    /// ```
    #[cfg(feature = "yaml")]
    pub fn from_yaml(y: &'key Yaml) -> ArgGroup<'key> { ArgGroup::from(y.as_hash().unwrap()) }
    ...
    pub fn arg(mut self, n: &'key str) -> Self {
        assert!(self.name != n,
    ...
    /// ```
    /// [arguments]: ./struct.Arg.html
    pub fn args(mut self, ns: &[&'key str]) -> Self {
        for n in ns {
            self = self.arg(n);
        }
        self
    ...
    pub fn requires(mut self, n: &'key str) -> Self {
    ...
    pub fn requires_all(mut self, ns: &[&'key str]) -> Self {
        for n in ns {
    ...
    pub fn conflicts_with(mut self, n: &'key str) -> Self {
    ...
    pub fn conflicts_with_all(mut self, ns: &[&'key str]) -> Self {
        for n in ns {
...
impl<'key> Debug for ArgGroup<'key> {
...
impl<'key, 'z> From<&'z ArgGroup<'key>> for ArgGroup<'key> {
    fn from(g: &'z ArgGroup<'key>) -> Self {
        ArgGroup {
            name: g.name,
            required: g.required,
            args: g.args.clone(),
...
#[cfg(feature = "yaml")]
impl<'key> From<&'key BTreeMap<Yaml, Yaml>> for ArgGroup<'key> {
    fn from(b: &'key BTreeMap<Yaml, Yaml>) -> Self {
...
impl<'key> Clone for ArgGroup<'key> {
    fn clone(&self) -> Self {
        ArgGroup {
            name: self.name,
