...
#[derive(Debug, Clone)]
pub struct ArgMatches<'key> {
    #[doc(hidden)]
    pub args: HashMap<&'key str, MatchedArg>,
    #[doc(hidden)]
    pub subcommand: Option<Box<SubCommand<'key>>>,
    #[doc(hidden)]
    pub usage: Option<String>,
}

impl<'key> Default for ArgMatches<'key> {
    fn default() -> Self {
        ArgMatches {
            args: HashMap::new(),
            subcommand: None,
...
impl<'key> ArgMatches<'key> {
    ...
    pub fn value_of_lossy<S: AsRef<str>>(&'key self, name: S) -> Option<Cow<'key, str>> {
        if let Some(arg) = self.args.get(name.as_ref()) {
            if let Some(v) = arg.vals.get(0) {
    ...
    pub fn values_of<S: AsRef<str>>(&'key self, name: S) -> Option<Values<'key>> {
        if let Some(arg) = self.args.get(name.as_ref()) {
    ...
    /// ```
    pub fn values_of_lossy<S: AsRef<str>>(&'key self, name: S) -> Option<Vec<String>> {
        if let Some(arg) = self.args.get(name.as_ref()) {
    ...
    /// [`String`]: https://doc.rust-lang.org/std/string/struct.String.html
    pub fn values_of_os<S: AsRef<str>>(&'key self, name: S) -> Option<OsValues<'key>> {
        fn to_str_slice(o: &OsString) -> &OsStr { &*o }
        let to_str_slice: fn(&'key OsString) -> &'key OsStr = to_str_slice; // coerce to fn pointer
        if let Some(arg) = self.args.get(name.as_ref()) {
    ...
    pub fn subcommand_matches<S: AsRef<str>>(&self, name: S) -> Option<&ArgMatches<'key>> {
    ...
    pub fn subcommand(&self) -> (&str, Option<&ArgMatches<'key>>) {
...
pub struct Values<'key> {
    iter: Map<Iter<'key, OsString>, fn(&'key OsString) -> &'key str>,
}

impl<'key> Iterator for Values<'key> {
    type Item = &'key str;

    fn next(&mut self) -> Option<&'key str> { self.iter.next() }
    fn size_hint(&self) -> (usize, Option<usize>) { self.iter.size_hint() }
}

impl<'key> DoubleEndedIterator for Values<'key> {
    fn next_back(&mut self) -> Option<&'key str> { self.iter.next_back() }
}

impl<'key> ExactSizeIterator for Values<'key> {}
...
#[derive(Clone)]
#[allow(missing_debug_implementations)]
pub struct OsValues<'key> {
    iter: Map<Iter<'key, OsString>, fn(&'key OsString) -> &'key OsStr>,
}

impl<'key> Iterator for OsValues<'key> {
    type Item = &'key OsStr;

    fn next(&mut self) -> Option<&'key OsStr> { self.iter.next() }
    fn size_hint(&self) -> (usize, Option<usize>) { self.iter.size_hint() }
}

impl<'key> DoubleEndedIterator for OsValues<'key> {
    fn next_back(&mut self) -> Option<&'key OsStr> { self.iter.next_back() }
}
